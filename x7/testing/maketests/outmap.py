...
from gg.devtools.maketests import inspect_more, mod_support
from gg.devtools.testing import annotations
from gg.devtools.maketests.parse import parse_module
from gg.devtools.maketests.types import OptStr, DictTested, DictAdded, ClassSrcInfo, Module
...
class OutputMap(object):
    ...
    def find_class_source(self, cls, lines):
        ...
        clines, start = inspect.getsourcelines(cls)
        start = start - 1           # Move from 1 based to 0 based counting
        last = start + len(clines)
    ...
    def build(self, module: Union[str, Module]):
        ...
        modlines, startline = inspect.getsourcelines(dst.module)
        if self.debug:  # pragma: no cover
            print('%s: %d lines @ %d' % (self.modname, len(modlines), startline))
        self.lines = [OutputMapElem(None, None, l) for l in modlines]
        ...
        for name, cls in dst.classes.items():
            ...
            if mod_support.is_local(cls, self.module):
                cstart, clast = self.find_class_source(cls, modlines)
                self.classes[cls.__name__] = ClassSrcInfo(cls, cstart, clast)
                for t in tests:
                    self.testmap[t] = cls.__name__
                    self.tested[t] = inspect_more.item_name(cls)
                if self.debug:  # pragma: no cover
                    print('    %s: %d lines @ %d. Ins @ %d' % (cls.__name__, clast-cstart, cstart, self.classes[cls.__name__].start))

                for ln in range(cstart, clast):
                ...
                for member, func in inspect.getmembers(cls, inspect.isfunction):
                    ...
                    flines, fstart = inspect.getsourcelines(func)
                    if self.debug:  # pragma: no cover
                        print('    %s: %d lines @ %d' % (member, len(flines), fstart))
                    for ln, l in enumerate(flines, fstart-1):
        ...
        # import_plain = fix_re(r'import (<names_as>)')
        # import_from = fix_re(r'from (<name>) import (<names_as>)')
